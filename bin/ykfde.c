...
int main(int argc, char **argv) {
	...
	char * device_names[2]; // magic number
	if ((device_name_config = iniparser_getstring(ini, "general:" CONFDEVNAME, NULL)) == NULL) {
		/* read from crypttab? */
		...
		goto out20;
				...
				char *device = strtok(device_name_config, " ");
				int device_num = 0;
				while (device != NULL)
				{
					fprintf(stdout, "loaded device: %s \n", device);
					device_names[device_num++] = device;
					device = strtok (NULL, " ");
				}
    }
    ...
    /* loop over all devices */
	    ...
	    if (cryptstatus != CRYPT_ACTIVE && cryptstatus != CRYPT_BUSY) {
	        ...
	        fprintf(stdout, "Device %s is valid.\n", device_names[i]);
	    }
